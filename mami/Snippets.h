...
public:
    ...
    Connection AddTask(const std::function<void()>& task);

    Connection AddTask(std::function<void()> && task);

    void DisconnectImpl(int connectionId);

    bool ValidImpl(int connectionId);
...
void ExecuteTasks(std::vector<Task> & taskQueue)
{
    if (taskQueue.empty()) {
        return;
    }
    std::vector<Task> currentTasks;
    std::swap(currentTasks, taskQueue);
...
Connection EventLoop::AddTask(const std::function<void()>& task)
{
    assert(task);
    Task newTask;
    newTask.on = task;
    taskQueue.push_back(std::move(newTask));
}

Connection EventLoop::AddTask(std::function<void()> && task)
{
    assert(task);
    Task newTask;
    newTask.on = std::move(task);
    taskQueue.push_back(std::move(newTask));
}
...
public:
    void Update(BehaiviorWrapper2 & behaivior)
    {
        if (static_cast<int>(sessions.size()) < maxConnectionCount) {
...
public:
    ...
    void Listen()
    {
        listenConn_ = service_->Schedule();
    }

public:
    IOService2* service_ = nullptr;
    ScopedConnection listenConn_;
    std::shared_ptr<BehaiviorWrapper2> listenBehaivior_;
