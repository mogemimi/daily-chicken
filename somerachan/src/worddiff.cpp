...
//    std::printf("\n");
    ...
    while ((row > 0) and (column > 0)) {
        ...
        if (longestCommonSubsequence
            and (*longestCommonSubsequence == mat(row, column))
            and (text1[row - 1] == text2[column - 1])) {
            DiffHunk hunk;
        ...
        if ((text1[row - 1] == text2[column - 1])
            and (equality < deletion)
            and (equality < insertion)) {
            DiffHunk hunk;
            hunk.text = text1[row - 1];
            hunk.operation = DiffOperation::Equality;
            hunks.push_back(std::move(hunk));
    ...
    std::reverse(std::begin(hunks), std::end(hunks));
    {
        ...
        for (auto & h : oldHunks) {
            if (hunks.empty() or (hunks.back().operation != h.operation)) {
